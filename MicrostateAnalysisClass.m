...
function runAnalysis(obj, sleepStages, sleepCycles)
    ...
    % Set default sleep stages if not provided
    if nargin < 2 || isempty(sleepStages)
        sleepStages = [1, 2, 3, 5];  % N1, N2, N3, REM (EXCLUDE WAKE)
    end
    if nargin < 3
        sleepCycles = [];
    end
    
    % ✅ CRITICAL FIX: FORCE REMOVAL OF WAKE FROM SLEEP STAGES
    if any(sleepStages == 0)
        fprintf('Removing wake stage from sleep stage analysis (sleep-focused study)\n');
        sleepStages = sleepStages(sleepStages > 0); % Remove wake (0)
        if isempty(sleepStages)
            error('No sleep stages remaining after removing wake. Please include at least one sleep stage.');
        end
    end
    ...
    % ✅ NEW: Create SLEEP-ONLY mask
    if ~isempty(obj.numericHypnogram)
        sleepOnlyMask = obj.createSleepOnlyMask(size(globalDataProc, 2));
        fprintf('Sleep-only analysis: %.1f%% of recording is sleep\n', mean(sleepOnlyMask)*100);
    else
        sleepOnlyMask = true(1, size(globalDataProc, 2)); % Use all data if no hypnogram
        fprintf('No hypnogram - using entire recording for global analysis\n');
    end
    
    % ✅ OPTIMIZED: Perform SLEEP-ONLY global microstate analysis
    sleepGlobalData = globalDataProc(:, sleepOnlyMask & ~globalArtifactMask);
    fprintf('Sleep-global analysis: %d clean samples (%.1f hours)\n', ...
        size(sleepGlobalData, 2), size(sleepGlobalData, 2)/fsNew/3600);
    
    sleepGlobalResults = obj.performMicrostateAnalysisOptimized(sleepGlobalData, fsNew);
    
    % ✅ OPTIMIZED: Sleep stage analysis - reuse preprocessed data
    stageResults = struct();
    if ~isempty(obj.numericHypnogram) && ~isempty(sleepStages)
        fprintf('Performing sleep stage analysis (using preprocessed data)...\n');
        stageResults = obj.performSleepStageAnalysisOptimized(globalDataProc, fsNew, sleepStages, globalArtifactMask);
    end
    ...
    % Transition probability analysis (on sleep-global data)
    transitionResults = obj.calculateTransitionProbabilities(sleepGlobalResults.segmentation, sleepGlobalResults.numMaps);
    
    % Store comprehensive results
    obj.microstateResults = struct(...
        'sleepGlobal', sleepGlobalResults, ...  % ✅ CHANGED: Sleep-only global
        'stageSpecific', stageResults, ...
    ...
    %% 1. SLEEP-GLOBAL MICROSTATE STATISTICS (UPDATED FIELD NAME)
    % ✅ FIX: Use 'sleepGlobal' instead of 'global'
    globalStats = {
    ...
    %% 2. INDIVIDUAL MICROSTATE MAPS (UPDATED FIELD NAME)
    ...
    %% 3. TRANSITION PROBABILITIES (UPDATED FIELD NAME)
    transData = {};
    for i = 1:R.sleepGlobal.numMaps
    ...
    if isfield(R, 'stageSpecific') && ~isempty(fieldnames(R.stageSpecific))
        ...
        for s = 1:length(stages)
            ...
            if ~isempty(stageResult) && isfield(stageResult, 'totalGEV')
                ...
                for i = 1:min(stageResult.numMaps, R.sleepGlobal.numMaps)
                    stageData{end, 4+i} = stageResult.mapCoverage(i) * 100;
                end
                
                % Fill remaining columns if stage has fewer maps
                if stageResult.numMaps < R.sleepGlobal.numMaps
                    for i = (stageResult.numMaps+1):R.sleepGlobal.numMaps
                        stageData{end, 4+i} = NaN;
                    end
                end
            end
        end
        ...
        if ~isempty(stageData)
            varNames = {'Stage', 'Total_GEV', 'Mean_Duration_ms', 'Mean_Occurrence_Hz'};
            for i = 1:R.sleepGlobal.numMaps
    ...
    if isfield(R, 'cycleSpecific') && ~isempty(fieldnames(R.cycleSpecific))
        ...
        for c = 1:length(cycles)
            ...
            if ~isempty(cycleResult) && isfield(cycleResult, 'totalGEV')
                ...
                for i = 1:min(cycleResult.numMaps, R.sleepGlobal.numMaps)
                    cycleData{end, 4+i} = cycleResult.mapCoverage(i) * 100;
                end
                
                % Fill remaining columns if cycle has fewer maps
                if cycleResult.numMaps < R.sleepGlobal.numMaps
                    for i = (cycleResult.numMaps+1):R.sleepGlobal.numMaps
                        cycleData{end, 4+i} = NaN;
                    end
                end
            end
        end
        ...
        if ~isempty(cycleData)
            varNames = {'Cycle', 'Total_GEV', 'Mean_Duration_ms', 'Mean_Occurrence_Hz'};
            for i = 1:R.sleepGlobal.numMaps
                varNames{end+1} = sprintf('Map_%d_Coverage_Percent', i);
            end
    ...
    %% 7. GFP PEAKS TABLE (UPDATED FIELD NAME)
    ...
    writetable(globalTable, outputFile, 'Sheet', 'Sleep_Global_Statistics'); % ✅ UPDATED SHEET NAME
    ...
    try
        ...
        if exist('sleep_cycles', 'file') == 2
            ...
            fprintf('sleep_cycles function not found, using simple detection\n');
            detected_cycles = obj.simpleSleepCycleDetection(nrem_hypnogram);
        end
        ...
        function [cleanData, artifactInfo] = performTargetedCleaning(obj, microstateIndices, microstateLabels)
            % Targeted cleaning for microstate channels
            ...
            [cleanData, artifactInfo] = obj.artifactDetector.fullDataCleaning(...
                dataToClean, cleaningLabels, obj.fs, obj.numericHypnogram);
        end
...
function stageResults = performSleepStageAnalysisOptimized(obj, globalDataProc, fsNew, sleepStages, globalArtifactMask)
    % Optimized sleep stage analysis using preprocessed data
    stageResults = struct();
    ...
    for i = 1:length(sleepStages)
        ...
        fprintf('  Stage %s: %.1f seconds of clean data\n', stageName, stageDuration);
        ...
        if stageDuration > 60 % At least 1 minute of clean data
            ...
            if ~isempty(stageResult)
                ...
                fprintf('  Analysis failed for stage %s\n', stageName);
            end
        else
            fprintf('  Insufficient clean data for stage %s analysis (%.1f s < 60 s)\n', stageName, stageDuration);
        end
    end
end
...
function cycleResults = performSleepCycleAnalysisOptimized(obj, globalDataProc, fsNew, sleepCycles, globalArtifactMask)
    ...
    fprintf('Sleep-only mask: %d/%d samples (%.1f%%) are sleep periods\n', ...
        ...
        function stageSampleMask = createStageSampleMask(obj, stage, totalSamples)
            ...
            % Create sample mask for specific sleep stage
    samplesPerEpoch = 30 * obj.fs;
...
function printSummaryStatistics(obj)
    ...
    R = obj.microstateResults.sleepGlobal;  % ✅ CHANGED: Use sleepGlobal
    
    fprintf('\n=== MICROSTATE ANALYSIS SUMMARY (SLEEP-ONLY GLOBAL) ===\n');
    ...
    fprintf('Sleep duration analyzed: %.1f hours\n', size(R.segmentation, 2)/R.fs/3600);
    ...
    if isfield(obj.microstateResults, 'stageSpecific') && ~isempty(fieldnames(obj.microstateResults.stageSpecific))
        fprintf('\nStage-specific analysis completed for:\n');
    ...
    if isfield(obj.microstateResults, 'cycleSpecific') && ~isempty(fieldnames(obj.microstateResults.cycleSpecific))
        fprintf('\nCycle-specific analysis completed for:\n');
