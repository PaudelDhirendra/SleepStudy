...
% SPINDLEGUI MATLAB code for SpindleGUI.fig
%      SPINDLEGUI, by itself, creates a new SPINDLEGUI or raises the existing
%      singleton*.
%
%      H = SPINDLEGUI returns the handle to a new SPINDLEGUI or the handle to
%      the existing singleton*.
%
%      SPINDLEGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPINDLEGUI.M with the given input arguments.
%
%      SPINDLEGUI('Property','Value',...) creates a new SPINDLEGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SpindleGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SpindleGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SpindleGUI
...
% --- Executes just before SpindleGUI is made visible.
function SpindleGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SpindleGUI (see VARARGIN)

% Choose default command line output for SpindleGUI
...
% UIWAIT makes SpindleGUI wait for user response (see UIRESUME)
...
spinmethod=get(handles.listbox1,'Value');
fspin=str2num(get(handles.edit14, 'String'));

% Set file identification parameters
xmlSuffix = get(handles.edit15, 'String');
allfilespindles(dataFolder,resultFolder, ...
    analysisSignals, referenceSignals, referenceMethod, ...
    cb_fig_compute_coherence, ecg_name, fspin, startFile, ...
    xmlSuffix, spinmethod);
